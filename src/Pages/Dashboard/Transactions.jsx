...
const Transactions = () => {
  ...
  const { data: OverView, isLoading: isLoadingOverview } =
    useGetPaymentHistoryOverviewQuery({ page: 1 });
  // table columns
  const columns = [
    // {
    //     title: '#Sl',
    ...
    },
    {
      ...
      key: "AppointmentId",
      render: (_, record) => (
        <p>{record?.AppointmentId?.date?.split("T")[0]}</p>
      ),
    },
    {
      title: "Patient Name",
    ...
    <div className="bg-[var(--bg-white)] p-4 rounded-md">
      ...
      <div className="grid-4 gap-3 my-4">
        {overviewData?.map((item, i) => (
          <div
            key={i}
            className="w-full h-full rounded-md p-4 py-8 bg-[var(--bg-gray-20)]"
          >
